#include "icon.h"
#include<iostream>
int ICON::GRID_SIZE = 5;


pair<string,ICON> pairArray[] =
{
    make_pair("stone",ICON("stone",12,94, 10, 10)),
    make_pair("end",ICON("end",35,94, 25, 22)),
    make_pair("towerpos",ICON("towerpos",36,69, 10, 10)),
    make_pair("tower",ICON("tower",13,1, 10, 10)),
    make_pair("monster",ICON("monster",1,26, 10, 10)),
};

map<string,ICON> ICON::GAME_ICON_SET(pairArray,pairArray+sizeof(pairArray)/sizeof(pairArray[0]));


ICON::ICON(string name, int x, int y, int w, int h){
    this->typeName = name;
    this->srcX = x;
    this->srcY = y;
    this->width = w;
    this->height = h;
}

ICON ICON::findICON(string type){
    map<string,ICON>::iterator kv;
    kv = ICON::GAME_ICON_SET.find(type);
    if (kv==ICON::GAME_ICON_SET.end()){

       cout<<"Error: cannot find ICON"<<endl;
       return ICON();
    }
    else{
        return kv->second;
    }
}
